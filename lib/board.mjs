...
//   BOARD_SYNTH_MODEL    rapporten         (standard = BOARD_MODEL)
//   BOARD_REASONING      'off' (standard, hurtigst) eller 'model' (modellens egen standard)
...
const SYNTH = () => env('BOARD_SYNTH_MODEL', MODEL());
...
// ─── OpenRouter ───
async function callOR(model, system, user, maxTokens, timeoutMs) {
  ...
  if (env('BOARD_REASONING', 'off') === 'off') body.reasoning = { enabled: false };
  const go = async (b) => {
...
async function askJSON(model, system, user, maxTokens, timeoutMs) {
  const t = await callOR(model, system, user, maxTokens, timeoutMs);
  try { return parseJSON(t); }
  catch (e) { return parseJSON(await callOR(model, system, user + '\n\nSvar KUN med JSON-objektet.', maxTokens, timeoutMs)); }
}
...
// ─── operationerne ───
export function health() { return { ok: true, live: !!KEY(), model: MODEL(), fast: FAST(), code: !!env('BOARD_ACCESS_CODE', '') }; }
...
export async function handle(body, meta) {
  ...
  if (op === 'synth') {
    ...
    return { res: cleanSynth(await askJSON(SYNTH(), q.system, q.user, 2200, 55000), ids) };
  }
